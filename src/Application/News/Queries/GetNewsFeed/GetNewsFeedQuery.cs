...
/// <summary>Backs the News Feed page's infinite scroll - newest first, optionally narrowed to one pipeline (RSS/API tab) and/or one country.</summary>
public sealed record GetNewsFeedQuery(
    ArticleSourceType? SourceType,
    string? Country,
    int Skip,
    int Count) : IRequest<IReadOnlyList<NewsArticleDto>>;
...
public sealed class GetNewsFeedQueryHandler : IRequestHandler<GetNewsFeedQuery, IReadOnlyList<NewsArticleDto>>
{
    ...
    {
        var filter = new NewsArticleFeedFilter(request.SourceType, request.Country, request.Skip, request.Count);
