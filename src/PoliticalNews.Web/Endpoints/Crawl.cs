...
using PoliticalNews.Application.Crawl.Commands.CreateOrUpdateRecurringJob;
...
using PoliticalNews.Application.Crawl.Queries.GetCrawlHistoryById;
using PoliticalNews.Application.Crawl.Queries.GetCrawlJobStatus;
...
public sealed class Crawl : IEndpointGroup
{
    ...
    [EndpointDescription(
        ...
        "job's Hangfire server is running (PoliticalNews.Worker), guarded by the same distributed lock.")]
    ...
    [EndpointDescription(
        ...
        "so PoliticalNews.Worker's next restart re-syncs every provider's job back to whatever that file says.")]
