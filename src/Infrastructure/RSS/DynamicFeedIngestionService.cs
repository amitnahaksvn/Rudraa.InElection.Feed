    ...
    public async Task RunAsync(string feedSourceId, CancellationToken cancellationToken)
    {
        ...
        try
        {
            ...
            _logger.LogInformation(
                "Dynamic feed {SourceCode} completed: {Total} items, {New} new, {Duplicate} duplicate ({DurationMs}ms)",
                feedSource.SourceCode, totalItems, newItems, duplicateItems, stopwatch.ElapsedMilliseconds);
        }
        catch (Exception ex) when (!cancellationToken.IsCancellationRequested)
        {
            ...
            _logger.LogError(ex, "Failed to ingest dynamic feed {SourceCode} ({FeedUrl})", feedSource.SourceCode, feedSource.FeedUrl);
