...
namespace PoliticalNews.Infrastructure.Mongo;

/// <summary>
/// Ensures every Mongo index (and, implicitly, every collection) exists before the host starts
/// serving requests/ticks. Registered by <c>AddInfrastructure</c> so both <c>PoliticalNews.Web</c>
/// and <c>PoliticalNews.Worker</c> get it for free - whichever process runs first against a brand
